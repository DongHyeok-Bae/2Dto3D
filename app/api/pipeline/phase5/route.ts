...
export async function POST(request: NextRequest) {
  ...
  const tracker = createPipelineTracker(5)
  ...
  try {
    ...
    if (promptVersion && promptVersion !== 'latest') {
      promptData = await getLatestPrompt(5, promptVersion)
    } else {
      const activePrompt = await getActivePrompt(5)
    ...
    const validation = validatePhaseResultSafe(5, result)
